...
export default function FeedTab({refreshTrigger, profile}: FeedTabProps) {
  ...
  const fetchPosts = async (userId: string | null, offset = 0) => {
    try {
      ...
      const {likesByPostId, commentsByPostId} = await fetchPostCounts(postIds);
  ...
  const listEmpty = isLoading ? (
    <View style={{marginTop: 24, width: "100%"}}>{renderSkeletonList()}</View>
  ) : (
